 ...
 *     was reported to corrupt the heap. The earlier "act un-stub
 *     corrupts the heap" finding turned out to be a layout-shift
 *     symptom of separate madc bugs (mud_prog comma-stmt body and
 *     real-typed global codegen), all fixed in develop now. Try
 *     un-stubbing these too once act() is stable.
 */
